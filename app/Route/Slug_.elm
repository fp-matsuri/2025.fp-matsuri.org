...
route =
    RouteBuilder.preRenderWithFallback { data = data, head = head, pages = pages }
...
data : RouteParams -> BackendTask FatalError (Response.Response Data ErrorPage.ErrorPage)
data routeParams =
    Plugin.MarkdownCodec.withFrontmatter Data
        ...
        |> BackendTask.onError
            (\_ ->
                BackendTask.succeed (Response.errorPage ErrorPage.notFound)
            )
