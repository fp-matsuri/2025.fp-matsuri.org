...
import Css exposing (..)
import Css.Extra exposing (fr, gap, grid, gridColumn, gridTemplateColumns)
...
viewTimetableItem timetableItem =
    case timetableItem of
        Talk talk ->
            div
                [ css
                    [ gridColumn (columnFromTrack (trackFromUuid talk.uuid))
                    , padding (px 10)
                    , borderRadius (px 10)
                    , fontSize (px 14)
                ...
                [ text (trackFromUuid talk.uuid |> .name)
